...
<h2>Цель работы</h2>
<p>&nbsp;&nbsp;&nbsp;&nbsp;Разработать сервис предоставляющий данные о погоде в городе Симферополе на момент запроса.<br>  В качестве источника данных о погоде используйте: http://openweathermap.org/. В состав сервиса входит: серверное приложение на языке С++ и клиентское приложение на языке Python.
...
<p>&nbsp;&nbsp;&nbsp;&nbsp;Кроме того я ознакомился с дополнительными настройками сервиса(http://openweathermap.org/api/one-call-api#other), которые передаются в строке параметров (аргументов).<br>
...
<ul>Также я добавил три дополнительных обработчика Get-запросов:
